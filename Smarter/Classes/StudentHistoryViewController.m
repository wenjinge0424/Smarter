...
@interface StudentHistoryViewController ()<UITableViewDataSource, UITableViewDelegate, MultiLineGraphViewDelegate, MultiLineGraphViewDataSource>
{
...
- (void)viewDidLoad {
    ...
    if (!self.student){
        self.student = [PFUser currentUser];
        btnBack.hidden = YES;
    } else {
        btnBack.hidden = NO;
    }
...
- (void) refreshItems {
    ...
    [query whereKey:PARSE_STUDY_OWNER equalTo:self.student];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    StudentHistoryItemViewController *vc = (StudentHistoryItemViewController *)[Util getUIViewControllerFromStoryBoard:@"StudentHistoryItemViewController"];
    UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    UILabel *lblDescription = (UILabel *)[cell viewWithTag:2];
    PFObject *obj = [dataArray objectAtIndex:indexPath.row];
    NSString *value = obj[PARSE_STUDY_STUDY_NUMBER];
    vc.studyNumber = value;
    vc.date = [Util getParseDate:obj.createdAt];
    vc.score = lblDescription.text;
