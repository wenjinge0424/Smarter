...
+ (NSInteger) getInterval{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSInteger interval = [defaults integerForKey:@"interval"];
    if (interval == 0){
        interval = 1;
    }
    return interval;
...
+ (NSString *) getParseDate:(NSDate *)date
{
    ...
    NSString *result = @"";
    if ([self isToday:date]){
        result = [NSString stringWithFormat:@"Today %@", [dateFormat stringFromDate:updated]];
    } else {
        result = [NSString stringWithFormat:@"%@", [dateFormat stringFromDate:updated]];
    }
...
+ (void) setConnection:(BOOL) isConnect userId:(NSString *)userId parentId:(NSString *)parentId teacherId:(NSString *) teacherId studentId:(NSString *)studentID
{
    ...
    [PFCloud callFunctionInBackground:@"setConnection" withParameters:data block:^(id object, NSError *err) {
        if (err) {
            NSLog(@"Fail to call 'setConnection'");
            return ;
        } else {
            NSLog(@"Success 'setConnection'");
        }
    }];
}

+ (void) playSound{
    [[self appDelegate].myAudioPlayer play];
}
